...
class TestOWDatasetBias(WidgetTest):
    ...
    def test_input_normal_data(self):
        ...
        self.assertEqual(self.widget.disparate_impact_label.text(), "The dataset is not suitable for bias computation.")
        self.assertEqual(self.widget.statistical_parity_difference_label.text(), "Pass the dataset through the 'As Fairness' widget first")
    ...
    def test_input_as_fairness_data(self):
        ...
        self.assertTrue(self.widget.disparate_impact_label.text().startswith("Disparate Impact (ideal = 1):"))
        self.assertTrue(self.widget.statistical_parity_difference_label.text().startswith("Statistical Parity Difference (ideal = 0):"))
