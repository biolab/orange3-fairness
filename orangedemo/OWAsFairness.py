...
class OWAsFairness(OWWidget):
    ...
    favorable_class = ContextSetting(None, schema_only=True)
    protected_attribute = ContextSetting(None, schema_only=True)
    privileged_values = ContextSetting([], schema_only=True)
    ...
    def __init__(self, *args, **kwargs):
        ...
        favorable_class_model = DomainModel(valid_types=(ContinuousVariable,))
        protected_attribute_model = DomainModel(valid_types=(ContinuousVariable,))
        # privileged_values_model = DomainModel(valid_types=(ContinuousVariable,))
        ...
        gui.comboBox(box, self, 'favorable_class', model=favorable_class_model, callback=self.commit.deferred, searchable=True,)

        box = gui.vBox(self.controlArea, 'Protected Attribute', margin=0)
        gui.comboBox(box, self, 'protected_attribute', model=protected_attribute_model, callback=self.commit.deferred, searchable=True,)

        box = gui.vBox(self.controlArea, 'Privileged Values', margin=0)
        gui.listBox(box, self, 'privileged_values', callback=self.commit.deferred, selectionMode=gui.QListView.ExtendedSelection,)
    ...
    def set_data(self, data: Table) -> None:
        ...
        if data:
            ...
            self.controls.favorable_class.model().set_domain(domain)
            self.controls.protected_attribute.model().set_domain(domain)
            self.controls.privileged_values.model().set_domain(domain)

            # Set the selected variables to the first variable in the list of variables (in the comboboxes)c
            self.favorable_class = self.controls.favorable_class.model()[0] if len(self.controls.favorable_class.model()) else None
            ...
            # Clear the list box and populate it with the new values
            self.privileged_values_listbox.clear()
            if domain is not None:
                for var in domain.variables:
                    self.privileged_values_listbox.addItem(var.name)
            ...
            if self.favorable_class is not None and self.protected_attribute is not None and self.privileged_values:
    ...
    def commit(self) -> None:
        if self._data is not None:
            self._data.attributes['favorable_class'] = str(self.favorable_class)
            self._data.attributes['protected_attribute'] = str(self.protected_attribute)
            self._data.attributes['privileged_values'] = [str(val) for val in self.privileged_values]
